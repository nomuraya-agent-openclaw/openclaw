  ...
  return html`
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          <div class="card-title">接続中のインスタンス</div>
          <div class="card-sub">ゲートウェイとクライアントからのプレゼンスビーコン。</div>
        </div>
      ...
      <div class="list" style="margin-top: 16px;">
        ${props.entries.length === 0
          ? html`<div class="muted">インスタンスがまだ報告されていません。</div>`
...
function renderEntry(entry: PresenceEntry) {
  const lastInput =
    entry.lastInputSeconds != null
      ...
      : "―";
  const mode = entry.mode ?? "不明";
  const roles = Array.isArray(entry.roles) ? entry.roles.filter(Boolean) : [];
  const scopes = Array.isArray(entry.scopes) ? entry.scopes.filter(Boolean) : [];
  const scopesLabel =
    scopes.length > 0
      ? scopes.length > 3
        ? `${scopes.length}個のスコープ`
        : `スコープ: ${scopes.join(", ")}`
      : null;
  return html`
    <div class="list-item">
      <div class="list-main">
        <div class="list-title">${entry.host ?? "不明なホスト"}</div>
      ...
      <div class="list-meta">
        ...
        <div class="muted">最終入力 ${lastInput}</div>
        <div class="muted">理由 ${entry.reason ?? ""}</div>
      </div>
    </div>
  `;
}
